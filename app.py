...
def get_targets_for_year(ach_df: pd.DataFrame, year: int):
    ...
    yearly = (
        float(ydf[ydf["month"] == 0]["target"].max())
        if not ydf[ydf["month"] == 0].empty
        else 0.0
    )
...
with st.sidebar:
    ...
    nav = st.radio(
        "",
        ["Dashboard", "Transactions", "Export", "Achievement"],
        index=0,
...
if add_sample:
    ...
    demo = [
        dict(
            id=nid,
            tx_date=start_m.isoformat(),
            project="Client A - TVC",
            tx_type="Income",
...
def add_net_cols(dfin: pd.DataFrame) -> pd.DataFrame:
    if dfin.empty:
        return dfin
    d = dfin.copy()
    d["base"], d["vat"], d["net"] = zip(
        *d.apply(lambda r: calc_amount(r["qty"], r["unit_price"], r["vat_percent"]), axis=1)
    )
    return d
...
if nav == "Dashboard":
    ...
    with a2:
        progress_block("ACHIEVEMENT (YEAR)", sales_ytd, annual_target, "ยังขาด")
    ...
    with left:
        ...
        st.markdown(
            "**Monthly Chart (Jan–Dec)** <span class='small-muted' style='float:right'>Sales vs Expense</span>",
            unsafe_allow_html=True,
        )
        ...
        if df_year.empty:
            ...
            m = months_df.merge(inc, on="month", how="left").merge(exp, on="month", how="left").fillna(0.0)
    ...
    with right:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown(
            "**Quick Export** <span class='small-muted' style='float:right'>CSV</span>",
            unsafe_allow_html=True,
        )
    ...
    with st.form("add_tx", clear_on_submit=True):
        ...
        with r1[0]:
            dte = st.date_input("วันที่", value=date.today())
        with r1[1]:
        ...
        with r3[2]:
            _, _, net = calc_amount(qty, unit_price, vat_percent)
    ...
    rows = [{"month": m, "month_name": months[m-1], "target": float(monthly_map.get(m, 0.0))} for m in range(1, 13)]
